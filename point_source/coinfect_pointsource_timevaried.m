% point source spread for varying outbreak timing (coinfection)
...
C_peaks = zeros(1, length(all_times));
C_peak_times = zeros(1, length(all_times));
...
for d_idx=1:length(all_times)
    ...
    S0=ones(N, 1); I0=zeros(N, 1); 
    J0=zeros(N, 1); C0=zeros(N,1);
    I0(1)=0.05;
    ...
    [C_peaks(d_idx), idx_peak] = max(nodes_C);
    C_peak_times(d_idx) = time_vals(idx_peak);
end

figure;
scatter(all_times, I_peaks, 40, 'filled');
xlabel('t_d');
ylabel('I Saturation Time');
filename = sprintf('I_peak_vs_td');
savefig(fullfile(output_folder, filename));

figure;
scatter(all_times, I_peak_times, 40, 'filled');
xlabel('t_d');
ylabel('J Saturation Time');
filename = sprintf('J_time_vs_td');
savefig(fullfile(output_folder, filename));
...
figure;
scatter(all_times, C_peaks, 40, 'filled');
xlabel('t_d');
ylabel('C Saturation Time');
filename = sprintf('C_peak_vs_td');
savefig(fullfile(output_folder, filename));

figure;
scatter(all_times, C_peak_times, 40, 'filled');
xlabel('t_d');
ylabel('C Saturation Time');
filename = sprintf('C_time_vs_td');
savefig(fullfile(output_folder, filename));
